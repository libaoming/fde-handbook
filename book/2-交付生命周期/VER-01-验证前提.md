...
2017 年 S3 故障期间，AWS 连自家状态页都更新不了——状态页的管理控制台恰好依赖
S3 本身（案例 PM-10）；2021 年 Roblox 三天余的全站故障里，用于定位故障的关键监控
就跑在出故障的 Consul 之上（案例 PM-04）。两次事故是同一个形态：观测手段与被观测
系统共享依赖，故障来临时它们一起消失——所以在相信任何验证结论之前，
先把验证手段自己的依赖审计一遍。
...
![验证链路与被验证系统共享依赖时，故障和观测一起消失——安静不是绿灯；防线是交集清单加心跳](../figures/ver01-blindspot.svg)
